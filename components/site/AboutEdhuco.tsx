...
const RESOURCES: ResourceItem[] = [
  ...
  },
  {
    ...
    title: "Audios",
    description:
      "Meditaciones, respiracion y sesiones de voz para tus rutinas.",
  },
  {
    icon: Video,
    title: "Videos",
      ...
      <div className="container-app relative z-10 space-y-16">
        <header className="mx-auto max-w-3xl space-y-6 text-center">
          ...
          <p className="mx-auto max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg">
            En <strong className="text-foreground">EDHUCO</strong> tienes un
            area personal donde accedes a
            ...
            <strong className="text-foreground">PDFs</strong> descargables para
            tu practica diaria. Puedes empezar gratis y, si quieres todo el
            contenido y funciones extra, suscribete por solo{" "}
            <strong
              className="bg-clip-text text-transparent"
              style={{ backgroundImage: WIPHLA_GRADIENT }}
            >
        ...
        >
          {RESOURCES.map((item, index) => {
            ...
            return (
              <article
                key={item.title}
                className="group relative rounded-2xl border bg-card/80 p-6 shadow-[0_28px_70px_-60px_rgba(17,24,39,0.65)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_36px_90px_-58px_rgba(17,24,39,0.72)] dark:bg-card/60"
                style={{
                  borderColor: hexToRgba(accent, 0.32),
                  background: `linear-gradient(135deg, ${hexToRgba(
                    accent,
                    0.08
                  )} 0%, ${hexToRgba(companion, 0.14)} 100%)`,
                }}
              >
                <div
                  className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                  style={{
                    backgroundImage: `radial-gradient(circle at top, ${hexToRgba(
                      accent,
                      0.16
                    )}, transparent 68%)`,
                  }}
                  ...
                  <span
                    ...
                    style={{
                      backgroundImage: `linear-gradient(135deg, ${hexToRgba(
                        accent,
                        0.24
                      )} 0%, ${hexToRgba(companion, 0.18)} 100%)`,
                    }}
                    aria-hidden="true"
                  >
        ...
        >
          <article
            className="relative rounded-3xl border border-white/20 bg-card/80 p-8 shadow-[0_32px_90px_-68px_rgba(17,24,39,0.75)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_40px_110px_-70px_rgba(17,24,39,0.82)] dark:border-white/10 dark:bg-card/60"
            style={{
              background: `linear-gradient(135deg, ${hexToRgba(
                WIPHLA_COLORS[2],
                0.08
              )} 0%, ${hexToRgba(WIPHLA_COLORS[6], 0.12)} 100%)`,
            }}
          >
            <div
          ...
          <article
            className="relative rounded-3xl border border-transparent bg-card/80 p-8 shadow-[0_36px_110px_-64px_rgba(17,24,39,0.85)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_44px_130px_-68px_rgba(17,24,39,0.9)] dark:bg-card/65"
            style={{
              ...
              background: `linear-gradient(135deg, ${hexToRgba(
                WIPHLA_COLORS[0],
                0.14
              )} 0%, ${hexToRgba(WIPHLA_COLORS[5], 0.18)} 100%)`,
            }}
          >
                ...
                <article
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${hexToRgba(
                      accent,
                      0.08
                    )} 0%, ${hexToRgba(companion, 0.12)} 100%)`,
                  }}
                >
                  <div
                    className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
